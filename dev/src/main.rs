...
#[tokio::main]
async fn main() {
    let bytes: Result<H256, _> = serde_json::from_str("\"0x00\"");
...
//     let mut stream = UnfoldingQueryStream::from_sql_api(logger, cheap_filter(), api);
//     let mut count = 0u64;
//     // consume the whole stream
//     let time = Instant::now();
//     while let Some(Ok(_data)) = stream.next().await {
//         count += 1;
//         info!(logger, "consumed")
//     }
//     info!(
//         logger,
//         "stream consumed {} block stream events in {}m",
//         count,
